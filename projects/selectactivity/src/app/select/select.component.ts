...
export class SelectComponent implements OnInit {
  ...
  doChoice(activity: any) {
    ...
    let choice: any = {
      ...
      Phone: this.phone,
      Email: this.email,
      PreferredDate:this.stringToDate(this.gibushDate,this.gibushTime, true)
    };
  ...
  stringToDate(dateStr: string, timeStr: string, enUs: boolean = false) {
    console.log(dateStr," ", timeStr)
    if (!dateStr && !timeStr) return '';
    const comps = dateStr.split('-');
    const comps2 = timeStr.split(':');
    if (!enUs) return ' תאריך מועדף: ' + comps[2] + '/' + comps[1] + '/' + comps[0] + comps2[0] + ':' + comps2[1] + ":00"
    console.log("comps:",comps)
    console.log(comps2)
    return comps[1] + '/' + comps[2] + '/' + comps[0] +" "+ comps2[0] + ':' + comps2[1] + ":00"
  }

  
  // stringToTime(timeStr: string, enUs: boolean = false) {
  //   if (!timeStr) return '';
  //   const comps = timeStr.split(':');
  //   if (!enUs) return ' שעה מועדפת: ' + comps[0] + ':' + comps[1] + ":00"
  //   console.log(comps)
  // }
